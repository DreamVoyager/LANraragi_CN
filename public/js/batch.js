...
Batch.initializeAll = function () {
    ...
    Server.callAPI("/api/archives", "GET", null, "无法加载完整的档案列表! 请重新加载页面.",
        (data) => {
...
Batch.checkUntagged = function () {
    Server.callAPI("/api/archives/untagged", "GET", null, "获取未标记的档案时出错!",
        (data) => {
...
Batch.startBatch = function () {
    ...
    Batch.socket.onmessage = function (event) {
        ...
        if (timeout !== 0) {
            $("#log-container").append(`睡眠 ${timeout} 秒.\n`);
        }
...
Batch.updateBatchStatus = function (event) {
    ...
    if (msg.success === 0) {
        $("#log-container").append(`处理时出错的 ID ${msg.id} (${msg.message})\n\n`);
    } else {
        switch (Batch.currentOperation) {
        case "plugin":
            $("#log-container").append(`已处理的 ID ${msg.id} with "${Batch.currentPlugin}" (添加标签: ${msg.tags})\n\n`);
            break;
        case "delete":
            $("#log-container").append(`删除的 ID ${msg.id} (文件名: ${msg.filename})\n\n`);
            break;
        case "tagrules":
            $("#log-container").append(`替换标签的 ID  ${msg.id} (新的标签: ${msg.tags})\n\n`);
            break;
        case "addcat":
            // Append the message at the end of this log,
            // as it can contain the warning about the ID already being in the category
            $("#log-container").append(`添加 ID ${msg.id} 到 ${msg.category} 分类! ${msg.message} \n\n`);
            break;
        case "clearnew": {
            $("#log-container").append(`清除了NEW标签的 ID ${msg.id}\n\n`);
        ...
        if (msg.title !== undefined && msg.title !== "") {
            $("#log-container").append(`将标题更改为: ${msg.title}\n`);
        }
    }
...
Batch.endBatch = function (event) {
    ...
    if (Batch.currentOperation === "delete") {
        $("#log-container").append("已删除档案, 将在5秒内重新加载页面...\n");
