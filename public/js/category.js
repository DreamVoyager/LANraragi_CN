...
Category.loadCategories = function (selectedID) {
    fetch(new LRR.apiURL("/api/categories"))
        ...
        .catch((error) => LRR.showErrorToast("从服务获取分类时出错", error));
};
...
Category.updateArchiveInCategory = function (id, checked) {
    ...
    Server.callAPI(`/api/categories/${categoryID}/${id}`, checked ? "PUT" : "DELETE", null, "添加/删除档案到分类时出错",
        () => {
...
Category.deleteSelectedCategory = function () {
    ...
    LRR.showPopUp({
        ...
        if (result.isConfirmed) {
            Server.callAPI(`/api/categories/${categoryID}`, "DELETE", "分类删除成功!", "删除分类时出错",
                () => {
...
Category.indicateSaving = function () {
    document.getElementById("status").innerHTML = "<i class=\"fas fa-spin fa-2x fa-compact-disc\"></i> 保存您的修改...";
};
...
Category.predicateHelp = function () {
    LRR.toast({
        ...
        heading: "编写关键词",
        text: "关键词遵循与存档索引中的搜索相同的语法. 参考 <a href=\"https://sugoi.gitbook.io/lanraragi/basic-operations/searching\">文档</a> 了解更多信息.",
        icon: "info",
        hideAfter: 20000,
    });
};
